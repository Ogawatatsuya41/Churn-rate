...
for fold, (train_index, valid_index) in enumerate(kf.split(X_std, y)):
    print(f"Fold {fold + 1}")
    ...
    lgb_train_scores.append(roc_auc_score(y_train, lgb_train_pred))
    lgb_valid_scores.append(roc_auc_score(y_valid, lgb_valid_pred))
    test_preds_lgb.append(lgb.predict_proba(X_test_std)[:, 1])

    # XGBoost (Optunaでのパラメータチューニング)
    ...
    xgb_train_scores.append(roc_auc_score(y_train, xgb_train_pred))
    xgb_valid_scores.append(roc_auc_score(y_valid, xgb_valid_pred))
    test_preds_xgb.append(best_xgb.predict_proba(X_test_std)[:, 1])

# 各foldの結果の平均
print("\nLightGBM Average Train Score:", np.mean(lgb_train_scores))
print("LightGBM Average Valid Score:", np.mean(lgb_valid_scores))
print("XGBoost Average Train Score:", np.mean(xgb_train_scores))
print("XGBoost Average Valid Score:", np.mean(xgb_valid_scores))

# テストデータに対する予測の平均
pred_lgbm = np.mean(test_preds_lgb, axis=0)
pred_xgb = np.mean(test_preds_xgb, axis=0)
pred_ensemble = (pred_lgbm + pred_xgb) / 2  # アンサンブル
